...
# Code -> name lookup (auto-generated from A_SHARE_POOL, no manual duplication)
_STOCK_NAME = {s["code"]: s["name"] for s in A_SHARE_POOL}
...
def get_market_regime(index_code: str = "000001", days: int = 120) -> Dict:
    ...
    try:
        ...
        # 统一列名：_sina_kline_direct 返回 "收盘"，akshare 返回 "close"
        close_col = "收盘" if "收盘" in idx_df.columns else "close"
        closes = idx_df[close_col]
